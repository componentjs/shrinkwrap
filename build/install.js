...
module.exports = function(program, cb) {
    ...
    var download = co(regeneratorRuntime.mark(function callee$1$0() {
        var dependencies, content, conflicts, downloads, repoUrl, entry, refKey, reference, originRef, hash, valid, i, item, oldPath, newPath;
        ...
        return regeneratorRuntime.wrap(function callee$1$0$(context$2$0) {
            ...
            case 0:
                if (typeof program.in === 'object') {
                    ...
                    content = fs.readFileSync(program.in, 'utf8');
                ...
                for (repoUrl in dependencies) {
                    entry = dependencies[repoUrl];
                    for (refKey in entry) {
                        reference = entry[refKey].version;
                        originRef = null;
                        hash = entry[refKey].resolved;
                        valid = semver.valid(reference);
                ...
                return context$2$0.abrupt("return", cb(conflicts));
            case 7:
            ...
            case 11:
                ...
                item = downloads[i];
                return context$2$0.delegateYield(downloader(item.repo, item.ref), "t0", 14);
            case 14:
                if (item.rename) {
                    oldPath = path.join(remoteComponents, item.repo, item.ref);
                    newPath = path.join(remoteComponents, item.repo, item.rename);
